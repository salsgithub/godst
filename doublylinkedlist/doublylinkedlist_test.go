...
func TestDoublyLinkedList_Prepend(t *testing.T) {
	...
	assertx.Equal(t, l.Values(), []int{2, 1})
	assertx.Equal(t, l.head, l.tail.previous)
	assertx.Equal(t, l.head.next, l.tail)
}
...
func TestDoublyLinkedList_Insert(t *testing.T) {
	...
	t.Run("insert in first half of the list", func(t *testing.T) {
		l := New(1, 2, 3, 4, 5, 6)
		inserted := l.Insert(2, 7)
		assertx.True(t, inserted)
		assertx.Equal(t, l.Values(), []int{1, 2, 7, 3, 4, 5, 6})
	})
}
...
func TestDoublyLinkedList_Remove(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "Remove at valid index in first half of the list removes that node",
			values:         []int{1, 2, 3, 4, 5, 6, 7},
			removalIndex:   2,
			expectedValue:  3,
			expectedRemove: true,
			expectedValues: []int{1, 2, 4, 5, 6, 7},
		},
		{
...
func TestDoublyLinkedList_At(t *testing.T) {
	...
	t.Run("valid index in first half of the list", func(t *testing.T) {
		l := New("the", "language", "is", "Go")
		value, ok := l.At(1)
		assertx.True(t, ok)
		assertx.Equal(t, value, "language")
		assertx.Equal(t, l.Len(), 4)
	})
	t.Run("valid index in second half of the list", func(t *testing.T) {
		l := New("the", "core", "language", "is", "Go")
		value, ok := l.At(3)
		assertx.True(t, ok)
		assertx.Equal(t, value, "is")
		assertx.Equal(t, l.Len(), 5)
	})
}
...
func TestDoublyLinkedList_Replace(t *testing.T) {
	...
	t.Run("valid index provided for replace in the first half of the list", func(t *testing.T) {
		l := New("the", "language", "is", "Go")
		l.Replace(1, "goal")
		value, ok := l.At(1)
		assertx.True(t, ok)
		assertx.Equal(t, value, "goal")
	})
	t.Run("valid index provided for replace in the second half of the list", func(t *testing.T) {
		l := New("the", "language", "to", "build", "good", "software", "with", "is", "Go")
		l.Replace(4, "great")
		value, ok := l.At(4)
		assertx.True(t, ok)
		assertx.Equal(t, value, "great")
	})
}

func TestDoublyLinkedList_Index(t *testing.T) {
...
func TestDoublyLinkedList_ReverseAll(t *testing.T) {
	t.Run("iterate in reverse for empty list", func(t *testing.T) {
		l := New[int]()
		var results []int
		for value := range l.ReverseAll {
			results = append(results, value)
			t.Error("iterator for an empty list should not yield any values")
		}
		assertx.Nil(t, results)
	})
	t.Run("iterate in reverse over a list with one node", func(t *testing.T) {
		l := New(1)
		var results []int
		for value := range l.ReverseAll {
			results = append(results, value)
	...
	t.Run("iterate in reverse over a list with multiple nodes", func(t *testing.T) {
		l := New("can", "you", "guess", "the", "language")
		var results []string
		for value := range l.ReverseAll {
			results = append(results, value)
		}
		assertx.Equal(t, results, []string{"language", "the", "guess", "you", "can"})
	})
	t.Run("iterate using reverse all directly", func(t *testing.T) {
		l := New(1, 2, 3)
		var results []int
		l.ReverseAll(func(i int) bool {
			if i > 1 {
				results = append(results, i)
				return true
			}
		...
		assertx.Equal(t, results, []int{3, 2})
	})
	t.Run("iterate using reverse all stops when yield returns false", func(t *testing.T) {
		l := New(1, 2, 3)
		var results []int
		l.ReverseAll(func(i int) bool {
			if i > 0 {
		...
		assertx.Equal(t, results, []int{3, 2, 1})
	})
}
...
func TestDoublyLinkedList_ReverseValues(t *testing.T) {
	l := New(1, 2, 3)
	values := l.ReverseValues()
	assertx.Equal(t, values, []int{3, 2, 1})
}
